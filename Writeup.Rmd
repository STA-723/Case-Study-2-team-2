...
We analyzed 48,895 AirBnB listings from New York City, New York. In particular, we attempted to identify factors that significantly contribute to a listing's popularity and price. Moreover, we explored heterogeneity among boroughs and neighborhoods with regards to room type, price, and popularity. Finally, using our results we provide examples of how to optimize popularity and price of a listing with respect to name and location. An exploratory data anlysis is done and various text analyses were performed on host and listing names to incorporate features such as sentiment, number of capital letters, and word length into our model. We use a hierchical random intercept model with neighborhood effects nested within borough effects, numerous fixed effects, and a CAR structure imposed at the neighborhood level. Model results are given in the appendix.
...
We placed $Gamma(.1,.1)$ priors on our precison parameters, and placed weakly informative normal priors on our coefficient/mean parameters. Also, we placed a uniform prior on $\rho$. Our final linear models can be represented as:
...
\end{center}
In order to maximize popularity, replace the location with "East Harlem". For a balance of both, replace the location with "Financial District". Note that both locations are also in Manhattan.
